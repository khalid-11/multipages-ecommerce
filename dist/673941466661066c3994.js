...
let totalPricesForAllProducts = document.getElementById(
...
itemsCount.forEach((item) => {
  item.addEventListener("change", (i) => {
    ...
    let thePriceArray = Array.from(totalPricesForEachProduct);
    let theTotalOfAll = thePriceArray.reduce((a, acc) => {
      return parseInt(a.textContent) + parseInt(acc.textContent);
    });
    console.log(theTotalOfAll);
    totalPricesForAllProducts.textContent = `${theTotalOfAll}$`;
  });
});
